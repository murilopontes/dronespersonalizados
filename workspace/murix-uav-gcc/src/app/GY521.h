...
//
// Developed by Murilo Pontes --  http://github.com/murix
// My great thanks to: Krodal and mbed team
//
// Driver for GY-521 Breakout board with support to Energia IDE
// look: http://energia.nu
//
...
//use standart Wire library of Energia IDE
#include "Wire.h"
...
public:
	...
	// x,y,z flat trim sums
	float gx_sum;
	float gy_sum;
	float gz_sum;
	float acc_pitch_sum;
	float acc_roll_sum;
	// x,y,z flat trim avgs
	float gx_avg;
	float gy_avg;
	float gz_avg;
	float acc_pitch_avg;
	...
	//
	float linear_speed_limit_x;
	float linear_speed_limit_y;
	float linear_speed_limit_z;
	//
	float linear_position_limit_x;
	float linear_position_limit_y;
	float linear_position_limit_z;
	//
	int acc_count_deadzone_limit;
	
};

class mpu6050_tracking_t {
public:
    // time tracking
	float imu_t_back;
	float imu_t_now;
	float imu_dt;
	// angles tracking
	float gyro_x_dt_back;
	float gyro_y_dt_back;
	float gyro_z_dt_back;
	float gyro_x_dt;
	float gyro_y_dt;
	float gyro_z_dt;
	float gyro_x_integrate_back;
	float gyro_y_integrate_back;
	float gyro_z_integrate_back;	
	float gyro_x_integrate;
	float gyro_y_integrate;
	float gyro_z_integrate;	
	float acc_pitch;
	float acc_roll;
	float fusion_pitch;
	float fusion_roll;	
	...
	float gravity_y;
	float gravity_z;
	float acc_linear_x[2];
	float acc_linear_y[2];
	float acc_linear_z[2];
	int acc_count_deadzone_x;
	int acc_count_deadzone_y;
	int acc_count_deadzone_z;
	float acc_linear_x_speed[2];
	float acc_linear_y_speed[2];
	float acc_linear_z_speed[2];
	float acc_linear_x_position[2];
	float acc_linear_y_position[2];
	float acc_linear_z_position[2];
};
...
public:	
	mpu6050_packet_t mpu6050_packet_raw;
	mpu6050_sensors_t sensors;
	mpu6050_calibration_t calibration;
	mpu6050_tracking_t tracking;
private:
    char currentAcceleroRange;
    char currentGyroRange;
	int _module; // stellaris/tiva - i2c port
	
public:
	GY521(int module);
	void init( void );
	int update();
	void flat_trim(int n_samples);
private:	
