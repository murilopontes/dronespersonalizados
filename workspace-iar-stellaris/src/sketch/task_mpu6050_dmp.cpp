...
//http://mbed.org/users/Sissors/code/MPU6050/file/5c63e20c50f3/MPU6050.cpp
...
//http://mbed.org/users/Sissors/code/MPU6050/file/5c63e20c50f3/MPU6050.h
...
static uint8_t mpu6050_currentGyroRange=0;
static uint8_t mpu6050_currentAcceleroRange=0;
...
void mpu6050_dmp_loop()
{
  ...
  float divisor_accel=1;
  switch(mpu6050_currentAcceleroRange){
    case MPU6050_ACCELERO_RANGE_2G: divisor_accel=16384.0;break;
    case MPU6050_ACCELERO_RANGE_4G: divisor_accel=8192.0;break;
    case MPU6050_ACCELERO_RANGE_8G: divisor_accel=4096.0;break;
    case MPU6050_ACCELERO_RANGE_16G: divisor_accel=2048.0;break;
  }
  
  double ax=accel_t_gyro.value.x_accel / divisor_accel * 9.81;
  ...
  Serial.print("} ");
    ...
    analogWrite(GREEN_LED,map((int)abs(az)*3,0,11*3,0,15));
  
    /////////////////////////////////////////////////////////////
    // x-axis
    if(ax>1){
       if(ax>11) ax= 11;
       analogWrite(PC_6,map((int)abs(ax),0,11,99,240));
       analogWrite(PC_7,min_pwn);
       analogWrite(PD_6,min_pwn);
       analogWrite(PD_7,min_pwn);
    }
    if(ax<1){
       if(ax<-11) ax= -11;
       analogWrite(PC_6,min_pwn);
       analogWrite(PC_7,map((int)abs(ax),0,11,99,240));
       analogWrite(PD_6,min_pwn);
       analogWrite(PD_7,min_pwn);
    }
    ///////////////////////////////////////////////////////////
    ...
    if(ay>1){
       ...
       analogWrite(PC_6,min_pwn);
       analogWrite(PC_7,min_pwn);
       analogWrite(PD_6,map((int)abs(ax),0,11,99,240));
       analogWrite(PD_7,min_pwn);
    ...
    if(ay<1){
       ...
       analogWrite(PC_7,min_pwn);
       analogWrite(PD_6,min_pwn);
       analogWrite(PD_7,map((int)abs(ax),0,11,99,240));
    }
    ////////////////////////////////////////////////////////
  ...
  float divisor_gyro=1;
  switch(mpu6050_currentGyroRange){
    case MPU6050_GYRO_RANGE_250: divisor_gyro=7505.7;break;
    case MPU6050_GYRO_RANGE_500: divisor_gyro=3752.9;break;
    case MPU6050_GYRO_RANGE_1000: divisor_gyro=1879.3;break;
...
void MPU6050_setGyroRange( char range ) {
    char temp;
    mpu6050_currentGyroRange = range;
    range = range & 0x03;
    temp = MPU6050_read_reg(MPU6050_GYRO_CONFIG_REG);
    temp &= ~(3<<3);
    temp = temp + range<<3;
    MPU6050_write_reg(MPU6050_GYRO_CONFIG_REG, temp);
}

void MPU6050_setAcceleroRange( char range ) {
    char temp;
    range = range & 0x03;
    mpu6050_currentAcceleroRange = range;
    
    temp = MPU6050_read_reg(MPU6050_ACCELERO_CONFIG_REG);
    temp &= ~(3<<3);
    temp = temp + (range<<3);
    MPU6050_write_reg(MPU6050_ACCELERO_CONFIG_REG, temp);
}

void MPU6050_setBW(char BW) {
    char temp;
    BW=BW & 0x07;
    temp = MPU6050_read_reg(MPU6050_CONFIG_REG);
    temp &= 0xF8;
    temp = temp + BW;
...
extern "C" void task_mpu6050_dmp(void *pvParameters){
  int error;
  uint8_t c;
  ...
  while(1){
       ...
       Serial.println("setup motors for ");
       Serial.print(time_setup);
       Serial.print("ms\r\n");
       analogWrite(PC_4,pwn_setup);
       analogWrite(PC_5,pwn_setup);
       analogWrite(PC_6,pwn_setup);  
       analogWrite(PC_7,pwn_setup);  
       vTaskDelay(time_setup);
       
       Serial.print("motors mininum speed for ");
       Serial.print(time_active);
       Serial.print("ms\r\n");
         
       analogWrite(PC_4,min_pwn);
       analogWrite(PC_5,min_pwn);
       analogWrite(PC_6,min_pwn);  
       analogWrite(PC_7,min_pwn);  
       vTaskDelay(time_active);
       time_active-=10;
       if(time_active < 10){
         time_active=200;
       }
